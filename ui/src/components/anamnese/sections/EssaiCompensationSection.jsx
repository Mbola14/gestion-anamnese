...
export default function EssaiCompensationSection({ data, onChange }) {
  ...
  const unifocalZone = data.unifocal_zone || { VL: false, VI: false, VP: false };
  const setUnifocalZone = (zoneKey, checked) => {
    handleChange("unifocal_zone", { ...unifocalZone, [zoneKey]: checked });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-xl border bg-slate-50 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Type de verres portés */}
          <div className="p-4 rounded-xl border bg-muted/30 space-y-3">
            <div className="font-medium">Type d’équipement</div>

            <TouchToggle
              options={[
                { value: "Unifocal", label: "Unifocal" },
                { value: "Vision_intermediaire", label: "Vision intermédiaire" },
            ...
            {/* Options supplémentaires seulement si Unifocal */}
            {data.type_verres_ancien === "Unifocal" && (
              <div className="rounded-xl border p-3 bg-white">
                <div className="text-sm font-medium mb-2">Zones (Unifocal)</div>
                <div className="flex flex-wrap gap-3">
                  <TouchCheckbox
                    label="VL"
                    checked={!!unifocalZone.VL}
                    onChange={(v) => setUnifocalZone("VL", v)}
                  />
                  <TouchCheckbox
                    label="VI"
                    checked={!!unifocalZone.VI}
                    onChange={(v) => setUnifocalZone("VI", v)}
                  />
                  <TouchCheckbox
                    label="VP"
                    checked={!!unifocalZone.VP}
                    onChange={(v) => setUnifocalZone("VP", v)}
                  />
                </div>
              </div>
            )}
